...
| C8 | **Named unlock owner** | Who physically unlocks the Mac after a power cut, and how they find out | Phase 1 item 3 / RUNBOOK | **Half answered · Sept 10** — the person is **Matthew R. Epstein**. *How he finds out* is still open, and it is the half that does the work: nothing currently notifies anyone that the machine is down, and the obvious channel (Telegram) does not exist yet (C2). Until then the detection method is "somebody notices" |
